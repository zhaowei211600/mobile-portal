...
$('#invoiceFull').load('/wechat/src/html/invoice.html',function () {
	clearInvoiceDetail()
	// //
	// $('#showGoodsListRow span').click(function () {
	// 	$('#saleListBox').show()
	// 	clearListDetail();
    ...
    if (count <= 8) {
        ...
        for (var i=0;i<count;i++) {
            ...
            if (i == 0) {
                ...
                $goodsDomCopy.find('.quantity').text( parseInt(goods.quantity) || '');
                $goodsDomCopy.find('.unitPrice').text( parseFloat(goods.unitPrice) || '');
